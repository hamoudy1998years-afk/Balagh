...
const ACCENT     = '#00F5C4';
const ACCENT_DIM = '#00F5C418';
const BG         = '#0A0A0F';
const CARD       = '#1A1A26';
const BORDER     = '#26263A';
const TEXT       = '#F0F0FF';
const SUBTEXT    = '#7070A0';
const MUTED      = '#2E2E44';
const DANGER     = '#FF4560';
...
function ASwitch({ value, onValueChange }) {
  return (
    <Switch
      ...
      thumbColor={value ? ACCENT : '#555570'}
...
function ThemeToggle({ isDark, onToggle }) {
  ...
  const trackBg = anim.interpolate({ inputRange: [0, 1], outputRange: ['#2E2E44', '#00F5C430'] });
  return (
...
export default function SettingsScreen({ navigation }) {
  ...
  if (screen === 'account') return (
    <SubScreen title="Account" onBack={() => setScreen(null)} insets={insets}>
      ...
      <Card>
        <Row icon="✏️" label="Edit Profile" sublabel="Name, bio, photo"
          onPress={() => { setScreen(null); navigation.navigate('EditProfile'); }} />
        <Row icon="🔑" label="Change Password" sublabel="Send a reset link to your email"
          onPress={handleResetPassword} />
        <Row icon="📧" label="Email Address" sublabel={currentUser?.email ?? 'Not set'}
          onPress={() => Alert.alert('Email', 'To change your email, please contact support.')} last />
      </Card>
      ...
      <Card>
        {editingPhone ? (
          ...
          <Row icon="📱" label="Phone Number"
            sublabel={phone || 'Tap to add a phone number'}
            onPress={() => setEditingPhone(true)} last />
        )}
      </Card>
      <GroupLabel text="LINKED ACCOUNTS" />
      <Card>
        <Row icon="🔄" label="Switch Account" sublabel="Manage multiple accounts"
          onPress={() => Alert.alert('Switch Account', 'Multiple account support coming soon!')} />
        <Row icon="➕" label="Add Account" sublabel="Log in to another account"
          onPress={() => Alert.alert('Add Account', 'Multiple account support coming soon!')} last />
      </Card>
      <GroupLabel text="DANGER ZONE" />
      <Card>
        <Row icon="🚪" label="Log Out" onPress={handleLogout} danger />
        <Row icon="🗑️" label="Delete Account" sublabel="Permanently remove your account"
  ...
  if (screen === 'privacy') return (
    <SubScreen title="Privacy & Safety" onBack={() => setScreen(null)} insets={insets}>
      ...
      <Card>
        {[
          ...
          <React.Fragment key={opt.value}>
            <TouchableOpacity
              style={styles.row}
              onPress={() => { setCommentPerm(opt.value); savePrivacy('comment_permission', opt.value); }}
            >
      ...
      <Card>
        <Row icon="❤️" label="Show Like Count"
          sublabel="Let others see your video like counts"
          right={<ASwitch value={showLikes} onValueChange={v => { setShowLikes(v); savePrivacy('show_likes', v); }} />}
          last
        />
      </Card>
      <GroupLabel text="SAFETY" />
      <Card>
        <Row icon="🚫" label="Blocked Users"
          sublabel="Manage accounts you've blocked"
          onPress={() => { setScreen('blocked'); loadBlockedUsers(); }}
          last
        />
      </Card>
    </SubScreen>
  ...
  if (screen === 'notifications') return (
    <SubScreen title="Notifications" onBack={() => setScreen(null)} insets={insets}>
      ...
      <Card>
        <Row icon="🔔" label="All Notifications"
          sublabel="Master toggle — turns everything on or off"
          right={<ASwitch value={notifAll} onValueChange={v => { setNotifAll(v); saveNotif('notif_all', v); }} />}
          last
        />
      </Card>
      <GroupLabel text="ACTIVITY" />
      <Card>
        <Row icon="💬" label="Comments"
          sublabel="When someone comments on your video"
          right={<ASwitch value={notifComments} onValueChange={v => { setNotifComments(v); saveNotif('notif_comments', v); }} />}
        />
        <Row icon="❤️" label="Likes"
          sublabel="When someone likes your video"
          right={<ASwitch value={notifLikes} onValueChange={v => { setNotifLikes(v); saveNotif('notif_likes', v); }} />}
        />
        <Row icon="👥" label="New Followers"
        ...
        />
        <Row icon="✉️" label="Direct Messages"
          sublabel="When someone sends you a message"
          right={<ASwitch value={notifMessages} onValueChange={v => { setNotifMessages(v); saveNotif('notif_messages', v); }} />}
          last
        />
      </Card>
    </SubScreen>
    ...
    <SubScreen title="Appearance" onBack={() => setScreen(null)} insets={insets}>
      ...
      <Card>
        <Row
          ...
          right={<ThemeToggle isDark={isDark} onToggle={() => setIsDark(p => !p)} />}
          last
        />
      </Card>
    </SubScreen>
  ...
  if (screen === 'faq') return (
    <View style={[styles.subContainer, { paddingTop: insets.top }]}>
      <View style={styles.subHeader}>
        <TouchableOpacity onPress={() => setScreen('help')} style={styles.subBack}>
          <Text style={styles.subBackIcon}>‹</Text>
        </TouchableOpacity>
      ...
      <ScrollView contentContainerStyle={[styles.subScroll, { paddingBottom: insets.bottom + 40 }]}>
        ...
        {[
          { title: '1. Acceptance of Terms',
            body: 'By downloading, installing, accessing, or using the Balagh mobile application, you agree to be bound by these Terms of Service and our Privacy Policy. These Terms constitute a legally binding agreement governed by the laws of the Republic of the Philippines. If you do not agree, you must immediately cease use of the App.' },
          { title: '2. Eligibility',
            body: 'You must be at least thirteen (13) years of age to use the Service. If you are under eighteen (18), you represent that you have obtained parental or guardian consent. You must have the legal capacity to enter into a binding contract and must not be barred from using the Service under Philippine law or any applicable jurisdiction.' },
          ...
          { title: '4. Prohibited Activities',
            body: 'You agree not to: post unlawful, defamatory, obscene, or harassing content; infringe upon intellectual property rights under Republic Act No. 8293 (Intellectual Property Code); engage in cyberbullying or harassment prohibited under Republic Act No. 10175 (Cybercrime Prevention Act of 2012); collect personal data without consent in violation of Republic Act No. 10173 (Data Privacy Act of 2012); spread misinformation that could incite violence; or attempt unauthorized access to the Service or any connected system.' },
          { title: '5. Content Ownership & License',
            body: 'You retain ownership of content you submit. By posting, you grant Balagh a non-exclusive, worldwide, royalty-free license to use, reproduce, and display your content in connection with the Service. We reserve the right to remove content that violates these Terms or applicable law without liability.' },
          { title: '6. Privacy',
            body: 'Your use of the Service is governed by our Privacy Policy, incorporated herein by reference. By using the Service, you consent to the collection, use, and sharing of your information in compliance with Republic Act No. 10173 (Data Privacy Act of 2012) and its Implementing Rules and Regulations.' },
          { title: '7. Disclaimer of Warranties',
            body: 'THE SERVICE IS PROVIDED ON AN "AS IS" AND "AS AVAILABLE" BASIS WITHOUT WARRANTIES OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING WARRANTIES OF MERCHANTABILITY OR FITNESS FOR A PARTICULAR PURPOSE. WE DO NOT WARRANT THAT THE SERVICE WILL BE UNINTERRUPTED, ERROR-FREE, OR FREE OF HARMFUL COMPONENTS.' },
          { title: '8. Limitation of Liability',
            body: 'TO THE MAXIMUM EXTENT PERMITTED BY APPLICABLE LAW, BALAGH SHALL NOT BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL, CONSEQUENTIAL, OR PUNITIVE DAMAGES ARISING FROM YOUR USE OF THE SERVICE. Nothing herein limits liability for gross negligence, fraud, or willful misconduct under applicable Philippine law.' },
          { title: '9. Indemnification',
            body: 'You agree to defend, indemnify, and hold harmless Balagh and its officers, directors, and employees from claims, liabilities, and expenses arising from your use of the Service, your posted content, your violation of these Terms, or your violation of applicable law or the rights of any third party.' },
          { title: '10. Governing Law & Dispute Resolution',
            body: 'These Terms are governed by the laws of the Republic of the Philippines. Any dispute shall be submitted to the exclusive jurisdiction of the proper courts of Davao City, Philippines. Prior to legal proceedings, parties agree to attempt good-faith negotiation for thirty (30) days.' },
      ...
      <ScrollView contentContainerStyle={[styles.subScroll, { paddingBottom: insets.bottom + 40 }]}>
        ...
        {[
          { title: '1. Information We Collect',
            body: 'We collect: (a) account information you provide such as username, email, password, and profile photo; (b) scholar application information including your real name, age, location, education, and expertise; (c) user-generated content such as videos, comments, and likes; (d) device information, usage data, and log data collected automatically when you use the Service.' },
          { title: '2. Legal Basis for Processing',
            body: 'We process your personal information based on: (a) your consent (Section 12(a), DPA; Article 6(1)(a), GDPR); (b) contractual necessity to provide the Service (Section 12(b), DPA; Article 6(1)(b), GDPR); (c) legal obligations (Section 12(c), DPA; Article 6(1)(c), GDPR); and (d) legitimate interests in operating and improving the platform (Section 12(f), DPA; Article 6(1)(f), GDPR).' },
          { title: '3. How We Use Your Information',
            body: 'We use your information to: create and manage your account; provide and improve the Service; process scholar verification applications; enable user interactions; send administrative communications; detect and prevent fraud and illegal activities; comply with legal obligations including NPC directives; and respond to your inquiries.' },
          { title: '4. Disclosure of Information',
            body: 'We do not sell your personal information. We may share information with: trusted service providers under confidentiality obligations; law enforcement or courts when required by Philippine law or NPC order; successor entities in a business transfer subject to data protection obligations; or third parties with your explicit consent.' },
          { title: '5. Scholar Profile Information',
            body: 'If you are a verified scholar, your full name, age, location, educational background, and expertise will be displayed publicly on your profile. By accepting scholar status, you expressly consent to this public display in accordance with the Data Privacy Act of 2012.' },
          { title: '6. Data Retention',
            body: 'We retain your personal information for as long as your account is active or as needed to provide the Service and comply with legal obligations. Upon account deletion, your personal data will be deleted within sixty (60) days, except where retention is required by law, in accordance with Section 11 of the DPA.' },
          { title: '7. Data Security',
            body: 'We implement appropriate technical and organizational security measures as required under Section 20 of the DPA, including encryption and access controls. In the event of a personal data breach, we will comply with mandatory notification requirements under the DPA and NPC regulations.' },
          ...
          { title: '11. International Data Transfers',
            body: 'Your personal information may be processed outside the Philippines. Where such transfers occur, we ensure appropriate safeguards are in place in accordance with Section 21 of the DPA IRR and applicable international data protection standards, including the GDPR.' },
...
const styles = StyleSheet.create({
  ...
  profileCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: CARD, borderRadius: 20, padding: 16, marginBottom: 28, borderWidth: 1, borderColor: BORDER },
  ...
  profileAvatarFallback: { width: 58, height: 58, borderRadius: 29, backgroundColor: '#7c3aed', alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: ACCENT },
  profileOnline:         { position: 'absolute', bottom: 1, right: 1, width: 13, height: 13, borderRadius: 7, backgroundColor: ACCENT, borderWidth: 2, borderColor: CARD },
  profileInfo:           { flex: 1 },
  profileName:           { fontSize: 16, fontWeight: '700', color: TEXT },
  profileHandle:         { fontSize: 13, color: ACCENT, marginTop: 2 },
  profileEditBtn:        { paddingHorizontal: 14, paddingVertical: 7, borderRadius: 20, borderWidth: 1.5, borderColor: ACCENT },
  profileEditText:       { fontSize: 13, fontWeight: '600', color: ACCENT },
  groupLabel:  { fontSize: 11, fontWeight: '800', letterSpacing: 1.8, color: SUBTEXT, marginBottom: 10, marginLeft: 4 },
  sectionDesc: { fontSize: 13, color: SUBTEXT, marginBottom: 12, marginLeft: 4, lineHeight: 19 },
  card: { backgroundColor: CARD, borderRadius: 18, borderWidth: 1, borderColor: BORDER, overflow: 'hidden', marginBottom: 24 },
  ...
  catChevron: { fontSize: 24, color: MUTED, fontWeight: '300' },
  catDivider: { height: 1, backgroundColor: BORDER, marginLeft: 76 },
  ...
  rowEnd:    { marginLeft: 8 },
  chevron:   { fontSize: 22, color: MUTED, fontWeight: '300' },
  rowDivider: { height: 1, backgroundColor: BORDER, marginLeft: 70 },
  logoutBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: DANGER + '15', borderRadius: 16, paddingVertical: 15, borderWidth: 1, borderColor: DANGER + '40', gap: 10, marginTop: 4, marginBottom: 8 },
  logoutIcon: { fontSize: 18 },
  logoutText: { fontSize: 16, fontWeight: '700', color: DANGER },
  subHeader:   { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: BORDER },
  ...
  phoneInput:      { backgroundColor: BG, borderRadius: 12, borderWidth: 1, borderColor: BORDER, color: TEXT, fontSize: 16, paddingHorizontal: 14, paddingVertical: 12, marginBottom: 12 },
  phoneActions:    { flexDirection: 'row', gap: 10 },
  phoneCancelBtn:  { flex: 1, paddingVertical: 11, borderRadius: 12, borderWidth: 1, borderColor: BORDER, alignItems: 'center' },
  ...
  phoneSaveText:   { color: BG, fontWeight: '700', fontSize: 15 },
  ...
  blockedRow:     { flexDirection: 'row', alignItems: 'center', backgroundColor: CARD, borderRadius: 16, padding: 14, marginBottom: 10, borderWidth: 1, borderColor: BORDER },
  ...
  unblockBtn:     { paddingHorizontal: 14, paddingVertical: 7, borderRadius: 20, borderWidth: 1.5, borderColor: ACCENT },
  unblockText:    { color: ACCENT, fontSize: 13, fontWeight: '600' },
  faqItem: { backgroundColor: CARD, borderRadius: 16, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: BORDER },
  ...
  legalSection:    { backgroundColor: CARD, borderRadius: 14, padding: 16, marginBottom: 10, borderWidth: 1, borderColor: BORDER },
